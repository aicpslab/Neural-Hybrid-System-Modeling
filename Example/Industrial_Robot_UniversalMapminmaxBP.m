...
%%
% 1. 创建网络
BPsingle = newff(Pn_train,Tn_train,[20 20]);

%%
% 2. 设置训练参数
BPsingle.trainParam.epochs = 500;
BPsingle.trainParam.goal = 1e-5;
BPsingle.trainParam.lr = 0.01;

%%
% 3. 训练网络
BPsingle = train(BPsingle,Pn_train,Tn_train);
...
BPswitch= newff(Pn_train,Tn_train,[10 6]);
[P1,ELMs]=MergePatitionsBP(P,BPswitch,e,mu',coeff(:,1:idx)',12);
size(P.intervals,2)
size(P1.intervals,2)
mse_switch = 0;
min_mse_switch=1;
...
for i =1:size(time_test,2)-2
    IN=mapminmax('apply',[y_test(:,i);y_test(:,i+1);u_test(:,i+1)],inputps);
     output_single(:,i) = sim(BPsingle,IN);
...
for i =3:duration
    ...
    Traj_singleN=sim(BPsingle,InputN);
