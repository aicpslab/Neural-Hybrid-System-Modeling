function [partitions,ELMs] = MergePatitionsBP(obj,BPs1,gamma,mu,coeff,inputdimension)
BPs1.trainParam.epochs = 10;
BPs1.trainParam.goal = gamma;
xs=obj.input;
t=obj.output;
...
while m1 <  lengthSegment
    ...
    while m2 <= lengthSegment
        ...
        ELMNetwork = trainELM(BPs1,inputData,outputData);
        k = k + 1;
...
for i = 1:size(inputDataSegmented,2)
    if(~isempty(TrainDatainput{1,i}))
       %  ELMs(i)=trainELMLipridge(ELMs1,inputDataSegmented{i}{1},outputDataSegmented{i}{1});
         ELMs(i)=trainELM(BPs1,inputDataSegmented{i}{1},outputDataSegmented{i}{1}); 
